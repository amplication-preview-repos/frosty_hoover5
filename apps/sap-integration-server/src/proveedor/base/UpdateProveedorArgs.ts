/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProveedorWhereUniqueInput } from "./ProveedorWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ProveedorUpdateInput } from "./ProveedorUpdateInput";

@ArgsType()
class UpdateProveedorArgs {
  @ApiProperty({
    required: true,
    type: () => ProveedorWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ProveedorWhereUniqueInput)
  @Field(() => ProveedorWhereUniqueInput, { nullable: false })
  where!: ProveedorWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ProveedorUpdateInput,
  })
  @ValidateNested()
  @Type(() => ProveedorUpdateInput)
  @Field(() => ProveedorUpdateInput, { nullable: false })
  data!: ProveedorUpdateInput;
}

export { UpdateProveedorArgs as UpdateProveedorArgs };
